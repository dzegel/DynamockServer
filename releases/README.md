...
|---|---|---|
|[3.0.0]()|Not Released|<ul><li>Rename file root from`Dynamock` to `DynamockServer`.</li><li>Handle admin port properly.</li><li>Remove deprecated `did_overwrite_response` property from `POST /expectations-suite/load` response.</li><li>Update default value of `dynamock.path.base` JAR argument to `dynamock`.</li></ul>|
